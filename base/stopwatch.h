...
{
public:
    stopwatch(): start_time(std::chrono::time_point::min()) {}
    ~stopwatch()
    {
        start_time = std::chrono::time_point::min();
    }
    ...
    bool is_started()
    {
        return(start_time == std::chrono::time_point::min());
    }

    //@return elapsed time(milliseconds) relative to start time
    uint64_t stop()
    {
        if(start_time != std::chrono::time_point::min()) {
